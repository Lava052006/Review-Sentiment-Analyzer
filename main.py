...
st.markdown("""
    ...
    .sentiment-card {
        ...
        min-height: 160px; /* Hard-coded height */
        display: flex;
        flex-direction: column;
        justify-content: center;
    }
    /* Lock the height of the notification slot to prevent layout jumps */
    .stAlert {
        min-height: 80px;
    }
    </style>
...
# ✅ Persistent message slot - fixed spot
...
if st.button("Analyze Sentiment"):
    ...
    st.rerun() # Ensure the UI updates immediately with the new status

# Render status from session state (stable placement)
msg_type, msg_text = st.session_state.status_msg
if msg_type == "info": status_area.info(msg_text)
elif msg_type == "warning": status_area.warning(msg_text)
elif msg_type == "error": status_area.error(msg_text)
elif msg_type == "success": status_area.success(msg_text)
...
if st.session_state.scraped_df is not None:
    ...
    with dashboard_container:
        ...
        # --- CENTERED PLOTLY CHART (Ultra Stable) ---
        # Data preparation for Plotly
        count_data = df['sentiment_label'].value_counts().reset_index()
        count_data.columns = ['Sentiment', 'Reviews']
